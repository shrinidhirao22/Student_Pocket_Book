...
function sendmail($email,$reset_token)
{
    ...
    try {
        ...
        <a href='http://localhost/PHPProject/WebDroids/faculty/resetforgotpass.php?email=$email&reset_token=$reset_token'> Reset Password</a>
...
if(isset($_POST['email']))
{
    ...
    if(!empty($email))
    {
        ...
        if($stmt->rowCount()===1)
        {
            ...
            $date=date("Y-m-d");
            $sql="update faculty set token='$reset_token',tokenexpire='$date' where email=?";
            $stmt=$conn->prepare($sql);
            $res=$stmt->execute([$email]);
            if($res && sendmail($email,$reset_token))
            {
                $sm = "Reset Link Sent Successfully!!";
                header("Location: ../faculty/forgotpass.php?success=$sm");
            }
            else
            {
                $em = "Reset Password Link Not Sent!!!";
                header("Location: ../faculty/forgotpass.php?error=$em");
            }
        }
        else
        {
