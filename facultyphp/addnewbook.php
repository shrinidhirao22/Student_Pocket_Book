...
if(isset($_SESSION['user_id']) && isset($_SESSION['user_email']))
{
    ...
    if(isset($_POST['btitle']) && isset($_POST['bdescription']) && isset($_POST['book_author']) && isset($_POST['book_courses']) && isset($_POST['book_branches']) && isset($_FILES['book_file']) && isset($_FILES['book_cover']))
    {
        ...
        sem_func($bsem,$text,$location,$ms,$user_input);
        ...
        {
            $em=$book_cover['data'];
            header("Location: ../faculty/addbooks.php?error=$em&$user_input");
            exit;
        }
        else
        {
            ...
            else
            {
                ...
                $sql="insert into books(isbn,title,author_id,description,course_id,branch_id,semester,faculty_id,cover,file,status) values(?,?,?,?,?,?,?,?,?,?,?)";
                $stmt=$conn->prepare($sql);
                ...
                if($res)
                {
                    ...
                    header("Location: ../faculty/addbooks.php?success=$sm");
                    exit;
                }
                else
                {
                    $em="Unknown Error occured!!!";
                    header("Location: ../faculty/addbooks.php?error=$em");
                    exit;
                }
            }
        }
    ...
    {
        header("Location: ../faculty/addbooks.php");
        exit;
    }
}
?>